...
```{r formula-1b, message=FALSE, fig.align='center'}
 ...
 ggplot(data = fly) +
   ...
   labs(x="Is it rude recline? ", title='f(RudeToRecline)') #+ guides(fill=guide_legend(title = "SleepHrsNight", reverse = TRUE))
```
...
ggplot(data = fly) +
   geom_mosaic(aes(x = product(DoYouRecline, RudeToRecline), fill=DoYouRecline, conds=product(Gender)), na.rm=TRUE, divider=mosaic("v")) + labs(x = "Is it rude recline? ", title='f(DoYouRecline, RudeToRecline| Gender)') + facet_grid(Gender~.)
