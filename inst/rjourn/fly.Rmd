...
These values are then sent through `productplots` functions to create the formula for the desired distribution: \texttt{weight $\sim$ alpha $+$ fill $+$ x $|$ conds }
...
ggplot(data = fly) + geom_mosaic(aes(x=product(RudeToBringUnrulyChild)), fill="#46B8DAFF", divider="hbar") +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank())  + labs(x="Is it rude to bring an unruly child?", title="Is it rude to bring an unruly child?")
```
...
```{r child2}
ggplot(data = fly) + geom_mosaic(aes(x=product(Gender, Child18), fill=RudeToBringUnrulyChild, alpha=Gender), na.rm=T, divider=ddecker()) +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank()) +labs(x="Gender, Child under 18", title="Is it rude to bring an unruly child?", subtitle="Broken down by whether respondent has a child under 18 and gender") +  scale_alpha_manual(values=c(0.9,0.7))
```
