...
\par The \texttt{ggmosaic} package was designed to create visualizations of categorical data. It is not limited to mosaic plots. Rather, it is capable of producing a wide variety of plots, including bar charts, stacked bar charts, spine plots, mosaic plots, and double decker plots. Each of these plots are constructed hierarchically, so the ordering of the variables is very important.
...
<<first_plot, echo = FALSE, message=FALSE, warning=FALSE, fig.cap="S .", fig.pos='h'>>=
...
\par What we see here is an interesting breakdown of participant responses to the two questions "Do you recline?" and "Is it rude to recline?". While some of the results were as expected - the more rude someone considers the act recling, the less likely they were to recline their own chair- there were three respondants who felt it was very rude to recline your seat on an airplane, yet always reclined their own seat.

<<talk, echo = FALSE, message=FALSE, warning=FALSE, fig.cap="S .", fig.pos='h'>>=
library(viridis)
ggplot(data = fly) + geom_mosaic(aes(x=product(Region), fill=RudeToTalkToNeighbor), na.rm=T) +
...
ggplot(data = fly) + geom_mosaic(aes(x=product(Child18), fill=RudeToBringBaby), na.rm=T) +
  guides(fill=guide_legend( reverse = TRUE))# +  scale_fill_viridis(discrete=TRUE, option="D", begin=.2, end=.9)

ggplot(data = fly) + geom_mosaic(aes(x=product(Child18), fill=RudeToBringBaby), na.rm=T) +
  guides(fill=guide_legend( reverse = TRUE)) +facet_grid(~Gender) #+  scale_fill_viridis(discrete=TRUE, option="D", begin=.2, end=.9)

ggplot(data = fly) + geom_mosaic(aes(x=product(Child18), fill=RudeToBringUnrulyChild), na.rm=T) +
  guides(fill=guide_legend( reverse = TRUE)) +  scale_fill_viridis(discrete=TRUE, option="D", begin=.2, end=.9)
