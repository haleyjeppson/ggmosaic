---
title: Mosaic Plots in the ggplot2 Framework ggmosaic
author:
  - name: Haley Jeppson
...
\texttt{ggmosaic} was created primarily using \texttt{ggproto} and the \texttt{productplots} package which was created by \citet{productplots, ieee-prodplots}. They refer to their framework as product plots, alluding to the computation of area as a product of height and width, and the statistical concept of generating a joint distribution from the product of conditional and marginal distributions.

To begin, \texttt{ggmosaic} began as a geom extension of the \texttt{rect} geom with the data handling provided in the \texttt{productplots} package which calculates \texttt{xmin}, \texttt{xmax}, \texttt{ymin}, and \texttt{ymax} for the \texttt{rect} geom to plot.
...
\texttt{ggmosaic} does not come without its own set of limitations and the main hurdle \texttt{ggmosaic} faces is that \texttt{ggplot2} is not capable of handling a variable number of variables. The current solution is to read in the variables \texttt{x1} and \texttt{x2} as \texttt{x = product(x1, x2)}. The \texttt{product} function creates a list of the variables specified which allows for it to pass \texttt{check\_aesthetics}, and then splits the variables back into a dataframe for the calculations.

The product function creates limitations for values the variables can take, and what the labels of variables can be. When the variables are combined, the values, variable name, and level are separated using ":", "-", and ".". For example, the \texttt{x} variable data is read is as \texttt{level-variable:value.level-variable:value}. If any of the variable names or values of the variable contain one of those 3 symbols, the function will break.
...
These values are then sent through `productplots` functions to create the formula for the desired distribution: \texttt{weight $\sim$ fill $+$ x $|$ conds }
...
```{r, echo = FALSE, message=FALSE, warning=FALSE,fig=TRUE, fig.show='hold', caption="", label=who1, width = 7, height = 7}
...
```{r who2, echo = FALSE, message=FALSE, warning=FALSE,fig=TRUE, fig.show='hold', fig.cap="Who?", width = 7, height = 7}
...
```{r who3, echo = FALSE, message=FALSE, warning=FALSE,fig=TRUE, fig.show='hold', fig.cap="I", width = 7, height = 7}
...
```{r recline1, echo = FALSE, message=FALSE, warning=FALSE,fig=TRUE, fig.cap="Is it rude to recline your seat?", width=7, height=14}

library(gridExtra)
a <- ggplot(data = fly) + geom_mosaic(aes(x=product(DoYouRecline)), fill="#D43F3AFF",na.rm=T, divider="hbar") +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank()) +labs(x="Is it rude to recline?", title="Do you recline your seat?")

b <- ggplot(data = fly) + geom_mosaic(aes(x=product(RudeToRecline)), fill="#EEA236FF", na.rm=T, divider="hbar") +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank()) +labs(x="Is it rude to recline?", title="Is it rude to recline your seat?") 
...
```{r recline, echo = FALSE, message=FALSE, warning=FALSE,fig=TRUE, fig.show='hold', fig.cap="Is it rude to recline your seat?", width=7, height=7}

ggplot(data = fly) + geom_mosaic(aes(x=product(RudeToRecline), fill=DoYouRecline), na.rm=T) +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank()) + labs(x="Is it rude to recline?", title="Do you recline your seat?", subtitle="Broken down by response to 'Is it rude to recline?'")  

#ggplot(data = fly) + geom_mosaic(aes(x=product(RecliningObligationToBehind), fill=DoYouRecline), na.rm=T)
```

```{r eliminate, echo = FALSE, message=FALSE, warning=FALSE,fig=TRUE, fig.show='hold', fig.cap="Is it rude to recline your seat?", width=7, height=7}


ggplot(data = fly) + geom_mosaic(aes(x=product(RudeToRecline), fill=EliminateReclining), na.rm=T) +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank()) + labs(x="Is it rude to recline?", title="Should reclining be eliminated?", subtitle="Broken down by response to 'Is it rude to recline?'") 
...
```{r baby, echo = FALSE, message=FALSE, warning=FALSE, fig=TRUE,  fig.cap="S .", fig.pos='h', width = 7, height = 7}

ggplot(data = fly) + geom_mosaic(aes(x=product(RudeToBringBaby)), fill= "#5CB85CFF", na.rm=T, divider="hbar") +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank())  +  labs(x="Is it rude to bring a baby?", title="Is it rude to bring a baby?") 
...
```{r baby2, echo = FALSE, message=FALSE, warning=FALSE, fig=TRUE,  fig.cap="S .", fig.pos='h', width = 7, height = 7}

ggplot(data = fly) + geom_mosaic(aes(x=product(Gender, Child18), fill=RudeToBringBaby), na.rm=T, divider=ddecker())+   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank()) +
...
```{r child, echo = FALSE, message=FALSE, warning=FALSE, fig=TRUE,  fig.cap="S .", fig.pos='h', width = 8, height = 5}

ggplot(data = fly) + geom_mosaic(aes(x=product(RudeToBringUnrulyChild)), fill="#46B8DAFF", divider="hbar") +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank())  + labs(x="Gender, Child under 18", title="Is it rude to bring an unruly child?")
```
...
```{r child2, echo = FALSE, message=FALSE, warning=FALSE,fig=TRUE,  fig.cap="S .", fig.pos='h', width = 7, height = 7}
ggplot(data = fly) + geom_mosaic(aes(x=product(Gender, Child18), fill=RudeToBringUnrulyChild), na.rm=T, divider=ddecker()) +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank()) +labs(x="Gender, Child under 18", title="Is it rude to bring an unruly child?", subtitle="Broken down by whether respondent has a child under 18 and gender")
```

```{r age_baby, echo = FALSE, message=FALSE, warning=FALSE,fig=TRUE,  fig.cap="S .", fig.pos='h', width = 7, height = 7}
ggplot(data = fly) + geom_mosaic(aes(x=product(Age), fill=RudeToBringBaby),                                  divider=ddecker(), na.rm=T) +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank()) + labs(x="Age Bracket", title="Is it rude to bring a baby on a plane?", subtitle="Responses broken down by age bracket")
```

```{r age_child, echo = FALSE, message=FALSE, warning=FALSE,fig=TRUE,  fig.cap="S .", fig.pos='h', width = 7, height = 7}
ggplot(data = fly) + geom_mosaic(aes(x=product(Age), fill=RudeToBringUnrulyChild),                                  divider=ddecker(), na.rm=T) +   scale_fill_locuszoom()+theme_hc() + theme(axis.text.y = element_blank()) + labs(x="Age Bracket", title="Is it rude to bring an unruly child on a plane?", subtitle="Responses broken down by age bracket")
```
